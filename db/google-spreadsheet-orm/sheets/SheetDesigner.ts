...
/**
 * Interface to interact with the Google Sheets
 *
 */
...
export class SheetDesigner extends SheetProvider {
  ...
  async getCollumns(worksheedId: number): Promise<string[]> {
    ...
    if (
      sM.data.matchedDeveloperMetadata &&
      sM.data.matchedDeveloperMetadata.length > 0
    ) {
      return sM.data.matchedDeveloperMetadata.map(
        item => item.developerMetadata.metadataValue
      );
    }
    return [];
  }
